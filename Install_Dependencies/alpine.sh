...
if [[ $? -eq 0 ]]; then
    ...
    tar -xzvf cmake-3.27.7-linux-aarch64.tar.gz
    mv cmake-3.27.7-linux-aarch64 /opt/cmake-3.27.7
    ...
    if [ -e "$binary_path" ]; then
        ...
        ln -s /opt/cmake-3.27.7/bin/cmake $binary_path
        echo "CMake $required_version symlinked to $binary_path from location in /opt."
    fi
    rm cmake-3.27.7-linux-aarch64.tar.gz
